...
FDeclaration::FDeclaration(const std::string& returnType, const std::string& identifier, Node* parameter, Node* block, Node* returnStatement) 
    : Node(eSymbolName::FDECL, identifier)
    , mBlock(block)
    , mReturnStatement(returnStatement)
    , mReturnType(DATA_TYPES.at(returnType))
{
    Node* current = parameter;
    while (!current->Empty()) 
    {
        const Parameter* currentParameter = dynamic_cast<Parameter*>(current);
